...
import { CheckoutForm, CheckoutFormErrors } from "../../models/CheckoutForm";

interface CheckoutProps {
  setOrderFormValue: React.Dispatch<
    React.SetStateAction<CheckoutForm | undefined>
  >;
}

const Checkout = ({ setOrderFormValue }: CheckoutProps) => {
  const [currentStep, setCurrentStep] = useState("details");
  const [formValues, setFormValues] = useState<CheckoutForm>({
    firstname: "",
    lastname: "",
    email: "",
    phonenumber: "",
  ...
  const setStep = (step: string, validationStep: string) => {
  ...
  const validateForm = (values: CheckoutForm, step = "payment") => {
    const numberRegex = /\d/;
      ...
      <article className='checkout-content'>
        {currentStep === "done" ? null : (
          <ul className='multi-steps'>
            <li>
              <span
                className={`step-number ${currentStep === "details" ? "active" : ""}`}>
                1
              </span>
            ...
            <li>
              <span
                className={`step-number ${currentStep === "address" ? "active" : ""}`}>
                2
              </span>
            ...
            <li>
              <span
                className={`step-number ${currentStep === "payment" ? "active" : ""}`}>
                3
              </span>
